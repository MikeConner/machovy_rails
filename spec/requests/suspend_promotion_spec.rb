  ...
  describe "Sign in and create new promotion" do
    ...
    describe "Switch to admin" do
      ...
      describe "should not have gone live" do
        ...
        it { should have_content("#{@promotion.status} [Suspended]")}
        
        describe "Vendor also sees that it's suspended" do
          before do
            visit root_path
            click_link 'Log out'
            all('a', :text => I18n.t('sign_in_register')).first.click
          ...
          it { should have_content("#{@promotion.title} [Suspended]")}
        end
      end
    end
  end
